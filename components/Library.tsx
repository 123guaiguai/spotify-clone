...
export default function Library({ setView, setGlobalPlaylistId }: any) {
                ...
                <div className='flex flex-wrap gap-6 mb-48'>
                    {playlists&&playlists.map((playlist) => {
                        return <div onClick={() => selectPlaylist(playlist)} key={playlist.id} className='cursor-pointer relative group w-56 mb-2 bg-neutral-800 hover:bg-neutral-600 rounded-md p-4'>
                            <div className='absolute opacity-0 group-hover:opacity-100 top-40 group-hover:top-36 transition-all ease-in-out duration-200 shadow-2xl shadow-neutral-900 z-10 h-12 w-12 flex items-center justify-center rounded-full bg-green-500 right-6'>
